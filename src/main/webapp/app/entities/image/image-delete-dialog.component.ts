...
@Component( {
                selector: 'jhi-image-delete-dialog', templateUrl: './image-delete-dialog.component.html'
            } )
    ...
    confirmDelete( id: number ) {
        this.imageService.delete( id ).subscribe( ( response ) => {
            this.eventManager.broadcast( {
                                             name: 'imageListModification', content: 'Deleted an image'
                                         } );
...
@Component( {
                selector: 'jhi-image-delete-popup', template: ''
            } )
