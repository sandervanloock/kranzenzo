...
public class WorkshopDateResourceIntTest {
    ...
    @Autowired
    private WorkshopDateSearchRepository workshopDateSearchRepository;
    ...
    public void setup() {
        ...
        final WorkshopDateResource workshopDateResource =
            new WorkshopDateResource( workshopDateRepository, workshopDateMapper, workshopDateSearchRepository );
    ...
    public void initTest() {
        workshopDateSearchRepository.deleteAll();
    ...
    public void updateWorkshopDate() throws Exception {
        ...
        WorkshopDate updatedWorkshopDate = workshopDateRepository.findOne( workshopDate.getId() );
    ...
    public void updateNonExistingWorkshopDate() throws Exception {
        ...
        // Validate the WorkshopDate in the database
        List <WorkshopDate> workshopDateList = workshopDateRepository.findAll();
        assertThat( workshopDateList ).hasSize( databaseSizeBeforeUpdate + 1 );
    }
    ...
    @Test
    @Transactional
    public void searchWorkshopDate() throws Exception {
        // Initialize the database
        workshopDateRepository.saveAndFlush( workshopDate );
        workshopDateSearchRepository.save( workshopDate );

        // Search the workshopDate
        restWorkshopDateMockMvc.perform( get( "/api/_search/workshop-dates?query=id:" + workshopDate.getId() ) )
                               .andExpect( status().isOk() )
                               .andExpect( content().contentType( MediaType.APPLICATION_JSON_UTF8_VALUE ) )
                               .andExpect( jsonPath( "$.[*].id" ).value( hasItem( workshopDate.getId().intValue() ) ) )
                               .andExpect( jsonPath( "$.[*].date" ).value( hasItem( sameInstant( DEFAULT_DATE ) ) ) );
